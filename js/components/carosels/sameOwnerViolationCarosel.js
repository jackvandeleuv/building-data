...
export class SameOwnerViolationCarosel {
    ...
    async load() {
        const url = `http://localhost:5000/same_owner_violations/${this.owner}`;
    ...
    makeLoadedCard(row, i) {
        const url = encodeURI(`${URI}?type=violation&record_id=${row.record_id}&parcelpin=${row.parcelpin}&owner=${row.deeded_owner_clean}`);
